...
import ListActivity from "../components/share-activities/ListActivity";
import {
  GetAllHomepagePostsSanityService,
  GetAllPostsSanityService,
  GetAllThanksSchoolsSanityService,
  GetAllWhatWeGotCardsSanityService,
  GetAllserviceCardSanityService,
  ResponseGetAllHomepagePostsSanityService,
  ResponseGetAllPostsSanityService,
  ResponseGetAllThanksSchoolsSanityService,
  ResponseGetAllWhatWeGotCardsSanityService,
  ResponseGetAllserviceCardSanityService,
} from "../sanity/services";
import MainFooter from "../components/footers/mainFooter";
import { Post } from "../sanity/sanity-models";
...
type PropsHome = {
  post: ResponseGetAllPostsSanityService;
  serviceCard: ResponseGetAllserviceCardSanityService;
  whatWeGotCards: ResponseGetAllWhatWeGotCardsSanityService;
  thanksSchools: ResponseGetAllThanksSchoolsSanityService;
  homepagePosts: ResponseGetAllHomepagePostsSanityService;
};
export default function Home({
  ...
  const [postsData, setPostsData] =
    useState<ResponseGetAllPostsSanityService>(post);
  ...
  const handleFectchMenu = async (index: number) => {
    setActiveMenu(index);
    posts.refetch();
  };
  ...
  const handleSelectedActivity = async ({ post }: { post: Post }) => {
    const arrayData = [post];
      ...
      <HomepageLayout>
        ...
        <div className="  ">
          <main className=" flex flex-col justify-center items-center h-full   bg-no-repeat bg-cover ">
            ...
            {homepagePosts.map((homepagePost, index) => {
                ...
                >
                  ...
                  >
                    <div className="xl:w-[25rem] xl:h-[25rem] md:w-[15rem] md:h-[15rem] w-80 h-80 group  relative">
                      <Image
                        src={homepagePost.mainImage.asset.url}
                        fill
                        ...
                        placeholder="blur"
                        blurDataURL={homepagePost.mainImage.asset.metadata.lqip}
                      />
                    </div>
                    <div
               ...
               h-max font-Kanit font-bold text-black text-4xl"
                    >
                      <div className="relative ">
                        <svg
                          className="absolute -top-5 -left-8 m-auto"
                          width="36"
                          height="37"
                          viewBox="0 0 36 37"
                          fill="none"
                      ...
                      <div className="text-lg font-normal mt-0 md:mt-2">
                        <PortableText
                          value={homepagePost?.description}
                          components={myPortableTextComponents}
                        />
                      </div>
                      <Link
                        href={`${homepagePost.link}`}
            ...
            {posts.isFetching ? (
              ...
              <ListActivity activityPosts={postsData} />
            )}
          </main>
...
export const getStaticProps: GetStaticProps = async (ctx) => {
  const post = await GetAllPostsSanityService();
  const homepagePosts = await GetAllHomepagePostsSanityService();
  const serviceCard = await GetAllserviceCardSanityService();
  const whatWeGotCards = await GetAllWhatWeGotCardsSanityService();
  const thanksSchools = await GetAllThanksSchoolsSanityService();
