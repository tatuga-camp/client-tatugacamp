...
import { SignInJWTService } from "../../../services/user";
...
function Index() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(() => true);
      const user = await SignInJWTService({
      ...
      if (user.user.IsResetPassword === true) {
        router.push(
          `/auth/new-password?access_token=${user.access_token}`,
          undefined,
          {
            shallow: true,
          }
        );
      } else if (user.user.IsResetPassword === false && user.schoolUser) {
        router.push(
          `${process.env.NEXT_PUBLIC_URL_SCHOOL}/school/dashboard?access_token=${user.access_token}`,
          undefined,
          {
            shallow: true,
          }
        ...
        router.push(
          `/classroom/?access_token=${user.access_token}`,
          undefined,
          {
            shallow: true,
          }
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="w-28  h-28 absolute animate-spin  -top-16 right-0 left-0">
              <Hands />
            </div>
          </div>
          <div className="mt-20">
