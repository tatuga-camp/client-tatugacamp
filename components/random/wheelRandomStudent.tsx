...
type WheelRandomStudentProps = {
  students: UseQueryResult<ResponseGetAllStudentsService, Error>;
  classroomId: string;
  user: User;
  setTriggerRouletteRandomStudent: any;
};

export type StudentWheel = {
  id: string;
  firstName: string;
  lastName: string;
  picture: string;
...
function WheelRandomStudent({
...
}: WheelRandomStudentProps) {
  const router = useRouter();
  const [firstRender, setFirstRender] = useState(false);
  const [selectedStudent, setSelectedStudent] = useState<
    StudentWheel & { option: string }
  >();
  ...
  const scores = useQuery({
    queryKey: ["scores"],
    queryFn: () => GetAllScoresClassroomService({ classroomId }),
  });
  ...
  useEffect(() => {
    ...
    if (shuffledArrayObject) {
      const parsedshuffledArrayObject: StudentWheel[] =
        JSON.parse(shuffledArrayObject);
      setStudentList(() => {
  ...
  const handleSpinClick = () => {
    if (!mustSpin) {
      ...
      const newPrizeNumber = Math.floor(
        Math.random() * (studentList?.length ?? 0)
      );
      ...
      <div className="w-max z-20 h-max rounded-2xl bg-white p-5 flex flex-col justify-center items-center gap-5">
        {studentList && studentList?.length > 0 ? (
          <Wheel
            ...
            data={studentList as (StudentWheel & { option: string })[]}
