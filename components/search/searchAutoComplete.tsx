...
type SearchAutoCompleteProps = {
  ...
  handleSelectedActivity: ({ post }: { post: Post }) => Promise<void>;
  searchFor: string;
};
function SearchAutoComplete({
  ...
  const [activites, setActivities] = useState<Post[]>(activityPosts);
  ...
  const filteredActivities =
    query === ""
      ? activites
      : activites.filter((activity: Post) =>
          activity.title
            .toLowerCase()
            .replace(/\s+/g, "")
  ...
  function handleSendSelectedActivity(activity: Post) {
    if (activity) {
      handleSelectedActivity({ post: activity });
    }
  }
  useEffect(() => {
      ...
      <Combobox value={selected} onChange={setSelected}>
        ...
        <div className="relative mt-1">
          <Combobox.Input
            ...
            displayValue={(activity: Post) => activity.title}
            ...
            >
              {filteredActivities.length === 0 && query !== "" ? (
                ...
                filteredActivities.map((activity) => (
                  <Combobox.Option
                    onClick={() => handleSelectedActivity({ post: activity })}
                    key={activity._id}
                    className={({ active }) =>
