...
import { GetAllClassroomsService } from "../../services/classroom";
import { CreateAssignmentToAnotherClassroomService } from "../../services/assignment";
import Loading from "../loadings/loading";

type AssignMultipleClassroomProps = {
  user: User;
  setTriggerAssignMultipleClassroom: React.Dispatch<
    React.SetStateAction<boolean>
  >;
};
function AssignMultipleClassroom({
  user,
  setTriggerAssignMultipleClassroom,
}: AssignMultipleClassroomProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [classroomState, setClassroomState] =
    useState<(Classroom & { isSelect: boolean })[]>();
  const classrooms = useQuery({
    queryKey: ["classrooms"],
    queryFn: () => GetAllClassroomsService(),
  });

  useEffect(() => {
    if (classrooms.isSuccess) {
  ...
  const handleChangeCheckbox = ({ classroomId }: { classroomId: string }) => {
  ...
  const handleClickToAssign = async () => {
    try {
      setLoading(() => true);
      if (!classroomState) throw new Error("โปรดเลือกห้องเรียน");
      ...
      for (const classroom of classrooms) {
        await CreateAssignmentToAnotherClassroomService({
