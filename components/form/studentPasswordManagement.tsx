...
type StudentPasswordManagementProps = {
  students: UseQueryResult<ResponseGetAllStudentsService, Error>;
  setTriggerStudentPasswordManagement: React.Dispatch<
...
function StudentPasswordManagement({
  ...
  students,
}: StudentPasswordManagementProps) {
  const [studentsCheckbox, setStudentCheckbox] =
  ...
  const handleChangeCheckbox = ({ studentId }: { studentId: string }) => {
  ...
  const handleStudentSummitResetPasswrod = async ({
    studentId,
  }: {
    studentId: string;
  }) => {
